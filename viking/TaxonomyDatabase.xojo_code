#tag Module
Protected Module TaxonomyDatabase
	#tag Method, Flags = &h0
		Sub CloseDatabase()
		  DB.Close
		End Sub
	...
	#tag Method, Flags = &h0
		Sub OpenDatabase()
		  Var source As FolderItem = SpecialFolder.Resource("taxonomy.sqlite")
		  ...
		  Try
		    ...
		    DB.DatabaseFile = SpecialFolder.ApplicationData.Child(name).Child("taxonomy.sqlite")
		    DB.Connect
	...
	#tag Method, Flags = &h0
		Function ReadSiteByName(name As String) As SiteDetails
		  Try
		    ...
		    Var siteDetails As New SiteDetails
		    
		    siteDetails.name = rs.Column("name").StringValue
	...
	#tag Method, Flags = &h0
		Sub ReadSiteNames(siteList As DesktopPopupMenu)
		  Try
	...
	#tag Method, Flags = &h0
		Sub WriteConfiguration(configuration As Configuration)
		  Try
